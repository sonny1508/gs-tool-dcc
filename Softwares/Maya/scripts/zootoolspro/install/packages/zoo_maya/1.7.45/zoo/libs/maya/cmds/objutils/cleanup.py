...
def deleteDisplayLayers():
    ...
    # Using MEL because it's more straightforward for this operation
    mel.eval('''
...
def deleteCameras():
    """Delete all cameras except the default ones"""
    # Get all cameras using MEL
    mel.eval('''
...
def deleteColorSets():
    """Delete all color sets from selected objects"""
...
def deleteUnusedMaterials():
    """Delete materials that aren't assigned to any objects in the scene, excluding Maya default materials"""
    default_materials = {
        'lambert1',
        'particleCloud1',
        'standardSurface1',
    ...
    all_shapes = cmds.ls(geometry=True) or []
    used_materials = set()
    ...
    all_materials = set(cmds.ls(materials=True) or [])
    unused_materials = all_materials - used_materials
    materials_to_delete = unused_materials - default_materials
