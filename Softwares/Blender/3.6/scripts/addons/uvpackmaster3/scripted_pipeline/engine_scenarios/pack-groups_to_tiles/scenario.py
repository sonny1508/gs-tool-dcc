...
class Scenario(PackScenario):
    ...
    def run(self):
        ...
        try:
            ...
            self.send_tip("the error can be easily fixed by setting 'Texel Density Policy' to 'Automatic' or 'Uniform'")
            raise
        ...
        for td_cluster in tdensity_clusters:
            ...
            for group in td_cluster.groups:
                ...
                if group.pack_to_single_box is not None:
                    stage_params.pack_to_single_box = group.pack_to_single_box

                stage = Stage()
                stage.params = stage_params
                stage.target = group.stage_target
