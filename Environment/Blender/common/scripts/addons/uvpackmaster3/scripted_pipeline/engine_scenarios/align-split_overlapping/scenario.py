...
from uvpm_core import packer, RetCode, LogType, IslandSet, OverlapDetectionMode
from utils import eprint
from scripted_pipeline import TO_ENUM
...
class Scenario(SimilarityScenario):
    ...
    def run(self):
        ...
        split_overlap_detection_mode = TO_ENUM(OverlapDetectionMode, self.cx.params['split_overlap_detection_mode'])
        ...
        while len(to_process) > 0:
            ...
            o_map = to_check.overlapping_map(split_overlap_detection_mode)
            # eprint(o_map)
            overlapping_islands = []
