...
from ...panel import UVPM3_PT_Registerable
...
class GroupingSchemeDrawer(GroupingSchemeAccess):
    ...
    def __init__(self,
                 ...
                 should_draw_presets=False):
        
        self.init_access(context, desc_id=access_desc_id, ui_drawing=True)
        self.scene_props = get_scene_props(context)
        self.active_mode = active_mode
        self.should_draw_grouping_options = should_draw_grouping_options
        self.should_draw_presets = should_draw_presets
    ...
    def draw_g_schemes(self, layout):
        ...
        op = row.operator(UVPM3_OT_NewGroupingScheme.bl_idname, icon='ADD', text='' if self.active_g_scheme is not None else UVPM3_OT_NewGroupingScheme.bl_label)

        if self.active_g_scheme is not None:
            op = row.operator(UVPM3_OT_RemoveGroupingScheme.bl_idname, icon='REMOVE', text='')

        if self.should_draw_presets:
            box = row.box()
            box.scale_y = 0.5
    ...
    def draw_target_boxes(self, layout):
        ...
        box_edit_UI = GroupingSchemeBoxesEditUI(self.context, self.scene_props)
...
class UVPM3_PT_GroupingBase(UVPM3_PT_Registerable):
    ...
    def poll(cls, context):
        return super().poll(context) and cls.active_mode.grouping_config.grouping_enabled
...
class UVPM3_PT_RequireGroupingScheme(UVPM3_PT_GroupingBase):
    ...
    @classmethod
    def poll(cls, context):
        return super().poll(context) and cls.active_mode.grouping_config.get_active_g_scheme(ui_drawing=True) is not None
...
class UVPM3_PT_GroupTargetBoxes(UVPM3_PT_RequireGroupingScheme):
    ...
    @classmethod
    def poll(cls, context):
        return super().poll(context) and cls.active_mode.grouping_config.target_box_editing
