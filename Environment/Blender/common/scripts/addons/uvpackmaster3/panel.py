...
class UVPM3_PT_Generic(bpy.types.Panel):
    ...
    def poll(cls, context):
        cls.prefs = get_prefs()
        cls.active_mode = cls.prefs.get_active_main_mode(context)
        return True
    ...
    def draw(self, context):
        ...
        self.active_mode = self.prefs.get_active_main_mode(context)
        self.context = context
    ...
    def prop_with_help(cls, obj, prop_id, layout, help_url_suffix=None):
        ...
        if not help_url_suffix:
            help_url_suffix = cls.HELP_URL_SUFFIX
...
class UVPM3_PT_SubPanel(UVPM3_PT_Registerable):
    ...
    def poll(cls, context):
        ...
        try:
            return cls.bl_idname in prefs.get_active_main_mode(context).subpanels_base()
...
class UVPM3_PT_IParamEdit(UVPM3_PT_SubPanel):
    ...
    def get_main_property(self):
        return PropertyWrapper(get_scene_props(self.context), self.IPARAM_EDIT_UI.ENABLED_PROP_NAME)
