...
class UVPM3_PT_SplitOverlapping(UVPM3_PT_SubPanel):
    ...
    def draw_impl(self, context):
        ...
        self.draw_enum_in_box(self.scene_props, "split_overlap_detection_mode", Labels.SPLIT_OVERLAP_DETECTION_MODE_NAME, col)

        from ..modes.align_mode import UVPM3_Mode_Align
        row = col.row(align=True)
        self.operator_attach_mode(UVPM3_OT_SplitOverlapping.bl_idname, UVPM3_Mode_Align.MODE_ID, row)

        row = col.row(align=True)
        self.operator_attach_mode(UVPM3_OT_UndoIslandSplit.bl_idname, UVPM3_Mode_Align.MODE_ID, row)
