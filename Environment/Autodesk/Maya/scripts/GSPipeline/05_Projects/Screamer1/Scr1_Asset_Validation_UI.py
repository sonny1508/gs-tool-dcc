...
def check(*args):
    ...
    if not selected_paths:
        pm.textScrollList("operationField", e=True, a="No objects selected. Please select objects to check.")
        return
    ...
    pm.textScrollList("operationField", e=True, a=f"Checking {len(mesh_objects)} mesh objects...")
    
    # 1) CHECK: All mesh must start with SM_
    pm.textScrollList("operationField", e=True, a="--- 1. CHECKING MESH NAMING (SM_) ---")
    ...
    for obj_path in mesh_objects:
        ...
        if not obj_name.startswith("SM_"):
            pm.textScrollList("operationField", e=True, a=f"{obj_name} does not start with SM_")
            has_naming_issues = True
    
    if not mesh_objects:
        pm.textScrollList("operationField", e=True, a="No mesh objects found in selection.")
    elif not has_naming_issues:
        pm.textScrollList("operationField", e=True, a="All mesh objects follow the naming convention.")
    ...
    pm.textScrollList("operationField", e=True, a="--- 2. CHECKING MATERIAL NAMING (MI_) ---")
            ...
            for se in shading_engines:
                try:
                    ...
                    if material_name.lower().startswith("lambert") or material_name.lower().startswith("standardsurface"):
                        pm.textScrollList("operationField", e=True, a=f"{obj_name} has wrongly assigned material: {material_name}")
                        has_material_issues = True
                    # Check if material starts with MI_
                    elif not material_name.startswith("MI_"):
                        pm.textScrollList("operationField", e=True, a=f"{material_name} does not start with MI_ ({obj_name})")
                        has_material_issues = True
                except Exception as e:
                    pm.textScrollList("operationField", e=True, a=f"Error checking material on {obj_name}: {str(e)}")
                    has_material_issues = True
    
    if not has_material_issues and mesh_objects:
        pm.textScrollList("operationField", e=True, a="All materials follow the naming convention.")
    ...
    pm.textScrollList("operationField", e=True, a="--- 3. CHECKING UV CHANNEL NAMES AND UV SHELLS ---")
    has_uv_issues = False
    ...
    for obj_path in mesh_objects:
        ...
        for shape in shapes:
            ...
            try:
                ...
                if not uv_sets:
                    pm.textScrollList("operationField", e=True, a=f"{obj_name} has no UV sets")
                    has_uv_issues = True
                    continue
                
                for uv_set in uv_sets:
                    if uv_set != "UV0" and uv_set != "UV1":
                        pm.textScrollList("operationField", e=True, a=f"{obj_name} has wrong UV channel name: {uv_set}")
                        has_uv_issues = True
                    ...
                    try:
                        ...
                        if shell_count == 0:
                            pm.textScrollList("operationField", e=True, a=f"{obj_name} has no UV shells in channel: {uv_set}")
                            has_uv_issues = True
                    except Exception as e:
                        pm.textScrollList("operationField", e=True, a=f"Error checking UV shells for {obj_name}: {str(e)}")
                        has_uv_issues = True
            except Exception as e:
                pm.textScrollList("operationField", e=True, a=f"Error checking UVs on {obj_name}: {str(e)}")
                has_uv_issues = True
    
    if not has_uv_issues and mesh_objects:
        pm.textScrollList("operationField", e=True, a="All UV channels follow the naming convention and have proper UV shells.")
    ...
    # 4) CHECK COMPLETE
    pm.textScrollList("operationField", e=True, a="--- 4. CHECK COMPLETE ---")
    
    # Summary
    pm.textScrollList("operationField", e=True, a="")
    pm.textScrollList("operationField", e=True, a=f"Checked {len(mesh_objects)} mesh objects.")
    if not has_naming_issues and not has_material_issues and not has_uv_issues and mesh_objects:
        pm.textScrollList("operationField", e=True, a="No issues found! All assets follow the required conventions.")
...
def exportSelectionAsFBX(*args):
    ...
    pm.textScrollList("operationField", e=True, ra=True)
    pm.textScrollList("operationField", e=True, a="Please save your Maya file before exporting...")
    ...
    if result != 'OK':
        pm.textScrollList("operationField", e=True, a="Export canceled.")
        return
    ...
    if not export_folder:
        pm.textScrollList("operationField", e=True, a="Export canceled - no folder selected.")
        return
    ...
    selected_paths = cmds.ls(selection=True, long=True)
    if not selected_paths:
        pm.textScrollList("operationField", e=True, a="No objects selected for export.")
        return
    ...
    if group_objects:
        ...
        for obj in group_objects:
            pm.textScrollList("operationField", e=True, a=f"  - {obj}")
    ...
    pm.textScrollList("operationField", e=True, a=f"Exporting {len(mesh_objects)} individual mesh objects to: {export_folder}")
    ...
    for obj_path in mesh_objects:
        ...
        try:
            ...
            pm.textScrollList("operationField", e=True, a=f"Exported: {obj_name}.fbx")
                
        except Exception as e:
            pm.textScrollList("operationField", e=True, a=f"Error exporting {obj_name}: {str(e)}")
...
def exportSelectionAsUV2FBX(*args):
    ...
    try:
        ...
        if not result_file:
            pm.textScrollList("operationField", e=True, a="Export canceled - no filename provided.")
            return
        ...
        pm.textScrollList("operationField", e=True, a="Error getting export filename.")
        return
    ...
    pm.textScrollList("operationField", e=True, a=f"Processing {len(mesh_objects)} mesh objects for export...")
    ...
    for obj_path in mesh_objects:
        ...
        shapes = cmds.listRelatives(obj_path, shapes=True, fullPath=True, type="mesh") or []
        if not shapes:
            pm.textScrollList("operationField", e=True, a=f"No mesh shape found for {obj_name}, skipping...")
            continue
        ...
        if not original_uv_sets:
            pm.textScrollList("operationField", e=True, a=f"{obj_name} has no UV sets, skipping...")
            continue
        ...
        if not dup_shapes:
            pm.textScrollList("operationField", e=True, a=f"No mesh shape found in duplicated {obj_name}, skipping...")
            continue
        ...
        try:
            ...
            if len(dup_uv_sets) >= 2:
                ...
                pm.textScrollList("operationField", e=True, a=f"For {obj_name}: Moved UV channel 2 to slot 1")
            else:
                # Case: Only one UV set - just rename it
                pm.textScrollList("operationField", e=True, a=f"For {obj_name}: Only one UV set found, keeping it")
            ...
            pm.textScrollList("operationField", e=True, a=f"Error processing UV sets for {obj_name}: {str(e)}")
    ...
    if temp_objects:
        ...
        try:
            ...
            fbx_command = f'FBXExport -f "{file_path_mel}" -s'
            mel.eval(fbx_command)
            pm.textScrollList("operationField", e=True, a=f"Exported combined FBX to: {result_file}")
                
        except Exception as e:
            pm.textScrollList("operationField", e=True, a=f"Error exporting: {str(e)}")
    else:
        pm.textScrollList("operationField", e=True, a="No valid objects to export after UV processing.")
    
    # Clean up - delete all temporary objects
    if temp_objects:
        cmds.select(temp_objects, replace=True)
        cmds.delete()
        # pm.textScrollList("operationField", e=True, a="Cleaned up temporary objects.")
    
    # Restore original selection
    cmds.select(original_selection)
    pm.textScrollList("operationField", e=True, a="Export with UVchannel2 complete.")
    ...
    pm.textScrollList("operationField", e=True, a="- Included: Smoothing Groups, Smooth Mesh, Triangulate")
    pm.textScrollList("operationField", e=True, a="- Excluded: Animation, Cameras, Lights, Audio, Embedded Media")
    pm.textScrollList("operationField", e=True, a="- UV Modification: Keep only UV Set 2 and renamed to UVChannel2")
    pm.textScrollList("operationField", e=True, a="- Export Type: Combined single FBX file")

def UI():
    if cmds.window("win", exists=True):
        cmds.deleteUI("win", window=True)
    
    cmds.window("win", title="Screamer1 Asset Validation (Python 3.7)")   
    ...
    pm.textScrollList("operationField", height=300)
