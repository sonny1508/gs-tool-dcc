...
# Logging Setup
logging.basicConfig()
logger = logging.getLogger("gs_path_manager")
...
class GSPathManagerDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=maya_main_window()):
        ...
        super(GSPathManagerDialog, self).__init__(parent)
    ...
    def showEvent(self, e):
        ...
        super(GSPathManagerDialog, self).showEvent(e)
        self.refresh_table

    def keyPressEvent(self, e):
        """ Key presses should not be passed to the parent """
        super(GSPathManagerDialog, self).keyPressEvent(e)
        e.accept()
    ...
    def build_gui_help_path_manager(self):
        """ Creates the Help GUI for GS Path Manager """
...
def try_to_close_gt_path_manager():
    """ Attempts to close GS Path Manager """
    try:
        gs_path_manager_dialog.close()  # pylint: disable=E0601
        gs_path_manager_dialog.deleteLater()
    except Exception as e:
        logger.debug(str(e))
...
if __name__ == "__main__":
    ...
    gs_path_manager_dialog = GSPathManagerDialog()
    gs_path_manager_dialog.show()


def launch():
    """Entry point for the GSTools menu/shelf."""
    global gs_path_manager_dialog
    try_to_close_gt_path_manager()
    gs_path_manager_dialog = GSPathManagerDialog()
    gs_path_manager_dialog.show()
