...
# Script Name
script_name = "GS - Outliner Sorter"
...
def build_gui_outliner_sorter():
    ...
    cmds.button(l="Move Top", c=lambda x: validate_operation("reorder_front"), w=130)
    cmds.separator(h=5, style='none')  # Empty Space
    cmds.rowColumnLayout(nc=2, cw=[(1, 130), (2, 130)], cs=[(1, 10), (2, 5)], p=content_main)
    cmds.button(l="Move Down", c=lambda x: validate_operation("reorder_down"), w=130)
    cmds.button(l="Move Bottom", c=lambda x: validate_operation("reorder_back"), w=130)
    cmds.separator(h=5, style='none')  # Empty Space
    cmds.rowColumnLayout(nc=2, cw=[(1, 130), (2, 130)], cs=[(1, 10), (2, 5)], p=content_main)
