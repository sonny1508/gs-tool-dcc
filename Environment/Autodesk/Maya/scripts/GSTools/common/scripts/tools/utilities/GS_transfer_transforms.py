...
# Logging Setup
logging.basicConfig()
logger = logging.getLogger("gs_transfer_transforms")
logger.setLevel(logging.INFO)

# Script Name
script_name = 'GS - Transfer Transforms'
...
def export_trs_transforms():
    ...
    if successfully_created_file and is_valid:
        export_dict = {'gs_transfer_transforms_version': script_version}
    ...
    if file_exists:
        try:
            with open(pose_file) as json_file:
                ...
                try:
                    if not data.get('gs_transfer_transforms_version'):
                        cmds.warning("Imported file doesn't seem to be compatible or is missing data.")
                    else:
                        import_version = float(re.sub("[^0-9]", "", str(data.get('gs_transfer_transforms_version'))))
                    ...
                    for obj in data:
                        if obj != 'gs_transfer_transforms_version':
                            # General Vars
                            found = False
                            found_obj = ''
                            long_name = obj
