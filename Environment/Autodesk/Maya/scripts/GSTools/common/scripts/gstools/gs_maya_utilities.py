...
def build_gui_about_gs_tools():
    """ Creates "About" window for the GS Tools menu """

    stored_gt_tools_version_exists = cmds.optionVar(exists="gs_tools_version")

    # Define Version
    if stored_gt_tools_version_exists:
        gs_version = cmds.optionVar(q="gs_tools_version")
    else:
        gs_version = '?'

    window_name = "build_gui_about_gs_tools"
    if cmds.window(window_name, exists=True):
        cmds.deleteUI(window_name, window=True)

    cmds.window(window_name, title="About - Glenda Studio (GS) Tools", mnb=False, mxb=False, s=True)
    ...
    cmds.text("Glenda Studio Tools", bgc=[.4, .4, .4], fn="boldLabelFont", align="center")
    cmds.separator(h=10, style='none', p="main_column")  # Empty Space

    cmds.rowColumnLayout(nc=1, cw=[(1, 300)], cs=[(1, 10)], p="main_column")
    cmds.text(l='Version Installed: ' + gs_version, align="center", fn="boldLabelFont")
    cmds.separator(h=5, style='none')  # Empty Space
    cmds.text(l='GS Tools is a collection of Maya scripts', align="center")

    cmds.separator(h=15, style='none')  # Empty Space
    cmds.text(l='About:', align="center", fn="boldLabelFont")
    cmds.text(l='This is collection scripts are owned by Glenda Studio.\n'
                'These scripts were created by Tan Phu and Long Nguyen,\n'
                'any copying or sharing is not allowed.', align="center")
    cmds.separator(h=15, style='none')  # Empty Space
