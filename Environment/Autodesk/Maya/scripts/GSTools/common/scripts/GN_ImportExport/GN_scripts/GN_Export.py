...
def GN_exportVertexColor(objects, file):
	...
	if vertexColor:
		...
		with open(file, 'w') as f:
			# Split file content by objects
			default = "g default"
			objects_data = fileContent.split(default)
			...
			for i in range(size):
				...
				# Re-build file content with the added MRGB block placed right after the vertex list
				if i != 0:
					search = re.search(r"^vt.+$\n", objects_data[i], re.MULTILINE).group()
					split = objects_data[i].split(search, 1)
					split[1] = mrgb_list[i-1] + search + split[1]
					objects_data[i] = "".join(split)
				if i < size - 1:
					objects_data[i] += default
				
				# Write file content
				f.write(objects_data[i])
...
def GN_PerformExport(filetype=1):
	...
	if filetype == 1:
		...
		cmds.FBXExportBakeComplexEnd("-v", 200)
		cmds.FBXExportBakeComplexStart("-v", 1)
		...
		cmds.FBXExportUseSceneName("-v", 0)
	...
	if any([filetype == 2, mode == 2, meshIntegrity == 3, uvSets > 1, freezeTransforms, combine, removeSets]):
		...
		if freezeTransforms:
			...
			for obj in objects:
				connections = cmds.listConnections(obj)
				if connections:
					connections = [conn for conn in connections if "modelPanel" not in conn]
					if len(connections):
	...
	if mode == 1:
		...
		if filetype == 1:
			...
			cmds.file(file, f=True, op="groups=1;ptgroups=1;materials=0;smoothing=1;normals=1", typ="OBJexport", pr=True, es=True)
		...
		# Print result
		GN_Print.GN_Print("# Result: Exported to " + file, deferred=True)
	...
	# Export Multi-File
		...
		# Print result
		message = "# Result: Exported {0} {1} file{2} in C:/temp/exported".format(objCount, fileFormat, "s" if objCount > 1 else "")
